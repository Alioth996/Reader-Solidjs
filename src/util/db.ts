...
class IDB {
  ...
  private initDB() {
    ...
    request.addEventListener('upgradeneeded', e => {
      ...
      if (!this.dbInstance.objectStoreNames.contains(this.storeName)) {
        // 创建对象仓库,类型新建 sqldb 里的表
        const objectStore: IDBObjectStore = this.dbInstance.createObjectStore(this.storeName, {
          keyPath: 'id',
          autoIncrement: true
        })
  ...
  add(data: any) {
    // indexedDB需要通过事务进行数据的crud操作
    const request: IDBRequest<IDBValidKey> = this.dbInstance
   ...
   */
  delete(key: number) {
    const request = this.dbInstance.transaction([this.storeName], 'readwrite').objectStore(this.storeName).delete(key)
    request.onsuccess = () => {
      console.log('数据删除成功')
    }
